          ...
          {assigments && assigments?.length > 0 && (
            <div className="flex flex-col sm:flex-row justify-between m-2">
              ...
              <div className="flex flex-col gap-6 mt-4 bg-white p-6 rounded-lg  border-2 border-[#66339975] w-full sm:w-1/2 h-auto sm:h-[120vh] overflow-y-auto">
                ...
                <h3 className="text-xl font-bold mb-2 text-[#663399b6] inline-block">{assignment.subject}</h3>
                <div className="absolute top-0 right-0 mt-1 mr-3 space-x-4">
                    <i className="fas fa-edit text-gray-500 mr-2 cursor-pointer" onClick={() => handleEdit(assignment.id)}></i>
                    <i className="fas fa-trash text-red-500 cursor-pointer" onClick={() => handleDelete(assignment.id)}></i>
                </div>
                <p className="text-gray-600 mb-2">Due Date: {new Date(assignment.dueDate).toLocaleDateString()}</p>
                <p className="text-gray-600 mb-2">Class: {assignment.forClass}</p>
                <p className="text-gray-600 mb-2">Created By: {assignment.createdBy}</p>
                <a href={assignment.docLink} target="_blank" rel="noopener noreferrer" className="text-blue-500 font-semibold mb-2 hover:underline block">View Document</a>
                <p className="text-gray-600">Created At: {new Date(assignment.createdAt).toLocaleString()}</p>
                </div>
